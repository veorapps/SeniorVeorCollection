...
export function HomePage({ data, featuredProducts }: HomePageProps) {
  ...
  return (
    <main>
      ...
      {data.featuredProducts.enabled ? <section className="py-9 lg:py-11"><Container><div className="flex flex-wrap items-end justify-between gap-4"><SectionHeading eyebrow={data.featuredProducts.eyebrow} title={data.featuredProducts.title} /><Link className="mb-1 inline-flex items-center gap-2 border-b border-brand-gold pb-1 text-[0.625rem] font-semibold tracking-[0.1em] text-brand-teal uppercase" href={data.featuredProducts.cta.href}>{data.featuredProducts.cta.label}<ArrowRight aria-hidden="true" className="size-4" strokeWidth={1.5} /></Link></div><div className="mt-6"><ProductCarousel cardClassName="w-[min(14.2rem,70vw)] [&_img]:aspect-[4/3]! [&_img]:object-contain" products={featuredProducts} /></div></Container></section> : null}
