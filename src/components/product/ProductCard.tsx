...
export function ProductCard({ className, onWishlistToggle, product, variant = "default", wishlistActive = false }: ProductCardProps) {
  ...
  return (
    <article className={cn("group relative overflow-hidden border border-brand-line bg-brand-paper", isShowcase ? "p-1.5 text-center" : "shadow-card", isMini ? "p-2" : isShowcase ? "" : "p-3", className)}>
      ...
      <div className={cn(isMini ? "pt-2" : isShowcase ? "px-1 pb-1 pt-2" : "pt-3")}>
        <Link className="block" href={`/parfumler/${product.slug}`}><h3 className={cn("font-display text-brand-ink", isMini ? "text-base" : isShowcase ? "text-[1.1rem]" : isCompact ? "text-lg" : "text-[1.05rem]")}>{product.name}</h3></Link>
        {!isMini ? <p className="mt-0.5 text-[0.625rem] font-medium tracking-[0.09em] text-brand-muted uppercase">{product.subtitle}</p> : null}
        {!isMini ? <RatingStars className={cn("mt-1.5", isShowcase && "justify-center")} rating={product.rating} reviewCount={product.reviewCount} showValue={false} /> : null}
        {isShowcase ? <><div className="mt-1 flex justify-center"><ProductPrice currency={product.currency} price={product.price} /></div><Link className="mt-1.5 flex min-h-7 items-center justify-center border border-brand-line text-[0.5625rem] font-semibold tracking-[0.1em] text-brand-ink uppercase" href={`/parfumler/${product.slug}`}>İncele</Link></> : <div className={cn("flex items-center justify-between", isMini ? "mt-1" : "mt-2")}><ProductPrice currency={product.currency} price={product.price} />{!isMini ? <Link className="border-b border-brand-gold text-[0.625rem] font-semibold tracking-[0.1em] text-brand-teal uppercase" href={`/parfumler/${product.slug}`}>İncele</Link> : null}</div>}
      </div>
    </article>
  );
}
