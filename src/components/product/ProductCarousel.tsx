  ...
  return (
    <div className="relative">
      <div aria-label="Ürün karuseli" className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 pr-12 [scrollbar-width:none]" ref={scrollContainerRef} role="region">
        {products.map((product) => <ProductCard className={cn(variant === "mini" ? "w-40 shrink-0 snap-start" : "w-[min(16.5rem,75vw)] shrink-0 snap-start", cardClassName)} key={product.id} product={product} variant={variant} />)}
      </div>
