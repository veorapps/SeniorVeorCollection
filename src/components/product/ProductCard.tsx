  ...
  return (
    <article className={cn("group relative overflow-hidden border border-brand-line bg-brand-paper shadow-card", isMini ? "p-2" : "p-3", className)}>
      ...
      <div className={cn(isMini ? "pt-2" : "pt-4")}>
        ...
        {!isMini ? <RatingStars className="mt-2" rating={product.rating} reviewCount={product.reviewCount} showValue={false} /> : null}
        <div className={cn("flex items-center justify-between", isMini ? "mt-1" : "mt-3")}><ProductPrice currency={product.currency} price={product.price} />{!isMini ? <Link className="border-b border-brand-gold text-[0.625rem] font-semibold tracking-[0.1em] text-brand-teal uppercase" href={`/parfumler/${product.slug}`}>İncele</Link> : null}</div>
      </div>
    </article>
  );
}
