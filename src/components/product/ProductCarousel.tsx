...
export function ProductCarousel({ cardClassName, products, variant = "compact" }: ProductCarouselProps) {
  ...
  return (
    <div className="relative">
      ...
      {isShowcase ? <><IconButton aria-label="Önceki ürünler" className="absolute left-[-3rem] top-1/2 hidden -translate-y-1/2 bg-brand-ivory lg:inline-flex" onClick={() => scrollByCard(-1)} size="sm"><ChevronLeft aria-hidden="true" className="size-5" strokeWidth={1.3} /></IconButton><IconButton aria-label="Sonraki ürünler" className="absolute right-[-3rem] top-1/2 hidden -translate-y-1/2 bg-brand-ivory lg:inline-flex" onClick={() => scrollByCard(1)} size="sm"><ChevronRight aria-hidden="true" className="size-5" strokeWidth={1.3} /></IconButton></> : <div className="absolute right-0 top-0 hidden gap-1 bg-brand-ivory pl-2 sm:flex"><IconButton aria-label="Önceki ürünler" onClick={() => scrollByCard(-1)} size="sm"><ChevronLeft aria-hidden="true" className="size-4" strokeWidth={1.5} /></IconButton><IconButton aria-label="Sonraki ürünler" onClick={() => scrollByCard(1)} size="sm"><ChevronRight aria-hidden="true" className="size-4" strokeWidth={1.5} /></IconButton></div>}
    </div>
  );
}
